...
bool RepoMSSQL::UpdateItem(Product &prod)
{
    ...
    query.prepare("update Product set idShipment=:idShipment,idSetter=:idSetter,g_ProductTypeId=:g_ProductTypeId"
                  ...
                  "g_garantMonth=:g_garantMonth where id=:id");
...
bool RepoMSSQL::UpdateItem(Modul &modul)
{
    ...
    query.prepare("update Modules set idShipment=:idShipment,idProduct=:idProduct,m_modTypeId=:m_modTypeId,m_name=:m_name,"
                  "m_number=:m_number,m_numberFW=:m_numberFW,m_dateCreate=:m_dateCreate,m_zip=:m_zip,m_garantMonth=:m_garantMonth "
                  "where id=:id");
...
void RepoMSSQL::FindItems(const QString &serialNumber, QList<Product> &listProduct, int status)
{
    ...
    if(serialNumber.isEmpty())
        return FindItems(listProduct, status);
...
void RepoMSSQL::FindItems(const QString &serialNumber, QList<Modul> &listModul, int status)
{
    ...
    if(serialNumber.isEmpty())
        return FindItems(listModul, status);
...
void RepoMSSQL::LoadClaimModules(int idClaim, QList<Modul> &listModul)
{
    ...
    query.prepare("select m.id,m.idShipment,m.idProduct,m_modTypeId,m_name,m_number,m_numberFW,m_dateEnd,m_dateCreate,m_zip "
                  "from ClaimModule cm "
